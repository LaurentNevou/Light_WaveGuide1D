...
% real index / imag index / thickness um / doping cm-3 / eff mass / Drude time ps
...
title(strcat('\lambda=',num2str(lambda*1e6,'%.3f'),'um; dy=',num2str(dy*1e9),'nm'))

xscale=get(gca,'xlim');
yscale=get(gca,'ylim');

x0=0.8*xscale(end);
y0=0.9*(yscale(end) - yscale(1)) + yscale(1);

text(x0,y0,s)

if TE==1
  sTE{1}='\fontsize{15}neff-TE';
  sTE{2}='-----------';  
  for i=1:length(neffTE)
    sTE{i+2}=strcat(num2str(neffTE(i)));
  end
  y0=0.7*(yscale(end) - yscale(1)) + yscale(1);
  text(x0,y0,sTE)
end

if TM==1
  sTM{1}='\fontsize{15}neff-TM';
  sTM{2}='-----------';
  for i=1:length(neffTM)
    sTM{i+2}=strcat(num2str(neffTM(i)));
  end
  y0=0.7*(yscale(end) - yscale(1)) + yscale(1);
  text(x0*1.1,y0,sTM)
end
